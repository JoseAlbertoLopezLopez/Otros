...
void filtradasPorCaso(){
	...
	for(int i=n*m; i>=limDownNs; i--){
		for(int j=(i>=limUpNs ? i-limUpNs : 0); j<=(i>=limUpNs ? limUpNs : i); j++){
			...
			filtradasCaso[{i-j, j}] = (size_t) ceil(casosAux/(n==m ? 6.0 : 3.0));
		...
		if(((double) 1.0*rand()/RAND_MAX)<=fraccion){
			...
			for(int j=0; j<=n-1; j++){
				...
				for(int k=0; k<=m-1; k++){
					...
					archivo << (listaVectores[i][j*m+k]==1 ? cambiosPorCaso[kk][i2][0] : (listaVectores[i][j*m+k]==2 ? cambiosPorCaso[kk][i2][1] : cambiosPorCaso[kk][i2][2])) << (k==m-1 ? ",0" : ",");
				}
				archivo << (j==n-1 ? "]" : "],");
			}   
			archivo << ",[" << repeat("0,",m+1) << "0]])." << endl;
...
//cout << omp_get_thread_num() << " " << &malla << endl;
	...
	double perm = (double) permutacionesPorCaso[{casosAux[k][0], casosAux[k][1]}]/(n==m ? 8.0 : 4.0);
