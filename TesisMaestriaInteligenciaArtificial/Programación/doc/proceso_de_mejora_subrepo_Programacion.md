...
- La carpeta `Matrices evaluadas/` contiene una base de datos de 941 MB.
...
Todos los archivos generados de extensión `.out`, `.log`, `.aux`, `.div`, entre otras, serán ignorados.

Los archivos generados, de extensiones `.txt`, `.tex`, `.gpl`, `.csv` y de tamaño menor a 100KB serán agregados, y los mayores o iguales a 100KB se les tomará una muestra.
...
Después del commit de creación, estos archivos generados se restaurarán a su versión original y se ignorarán.
Esto debido a que, además del gran tamaño de algunos, muchos de estos archivos son generados y la gran mayoría de estos ya no son útiles, por lo que serán eliminados definitivamente del repositorio local en commits posteriores a la creación del repositorio, como parte del proceso de mejora.
Por lo cual no tiene caso arrastrar el tamaño original de estos solo en los primeros commits.

En un archivo `.csv` añadido después del commit de creación, se documentarán todos los nombres de los archivos del directorio de trabajo antes de la creación del repositorio, junto con sus rutas, tamaños originales, entre otros datos.
...
A continuación se presenta una síntesis de cuáles archivos (excepto los de las extensiones ignoradas mencionadas) serán agregados, ignorados o se les tomará una muestra:

- [X] **Word's:** Se incluirán como documentación.

- [X] **Mallas PDF:** Aunque se trata de archivos generados, se incluirán todos como un tipo alternativo de documentación o visualización de algunos resultados, excepto `4x4 Cubos 5, Prismas 6.pdf` por su tamaño.

- [X] **Matrices evaluadas:** Se tomará muestra del archivo `matrices_evaluadas_ordenadas.csv`.

- [X] **Archivos casos:** Se tomarán muestras de los archivos de tamaño mayor a 100KB.

- [X] **Gnuplot:** No se ignorará ningún archivo, ya que son necesarios para la generación de gráficas.

- [X] **Bases de conocimiento:** Se tomará muestra del archivo `baseConocimientoMallasFiltradas.pl`.

  - [X] **Muestras 5x5:** Se tomarán muestras de todos los archivos.
  El archivo `Seguimiento.txt` se mantendrá completo.

  - [X] **4x4:**
    - Se mantendrán completos los archivos `.txt` con nomenclatura de casos en su nombre menores o iguales a 100KB, de los demás se tomarán muestras.
    - Se tomarán muestras de los archivos `baseConocimientoMallasFiltradas`$n$`.pl` y `baseConocimientoMallasFiltradas.pl`.
    - Se tomarán muestras de los archivos `EvaluacionMallas.txt` y `EvaluacionMallas`$n$`.txt`.
    - Se mantendrán completos los archivos `LimitesPasos`$n$`.txt`, así como el archivo `LimitesPasos.txt`.

    - [X] **Latex (mallas 10x10 pdfs):**
      - Se tomará muestra del archivo `01MallasLatex.tex`.
      - Se tomarán como muestra algunos archivos `Mallas`$n$`-eps-converted-to.pdf` completos.

  - [X] **3x3:** Se mantendrán completos los dos archivos.

- [X] **Versiones anteriores:** Se incluirán todos los archivos para tener registro de las versiones anteriores en los commits iniciales.

- [X] **Pseudocódigos:** Se incluirán todos los archivos monitoreables.

- [X] **LatexGnuplot:**
  Se tomarán muestras de todos los archivos `GraficadorMatricesGnuplot.gpl`.
  Solamente, se ignorarán archivos `.eps` y `.pdf` grandes, tomando algunos archivos completos de menor tamaño como muestras.

- [X] **Algoritmo de renderizado:** Se agregarán los tres scripts.

- [X] **Imágenes Word:** Se agregarán todas las imágenes.

- [X] **Puntajes:** Se agregarán los dos archivos.
...
Esto en el contexto del estado del repositorio local antes de su commit de creación (aunque debido a que en los primeros commits del repositorio muchos de los archivos y documentos no son útiles, serán pocos los casos en que se requiera volver a generarlos).
...
La siguiente es una lista de los archivos generados más importantes, junto con los scripts que pueden volver a generarlos a su tamaño original (no al de las muestras), correspondiente a su estado en el directorio de trabajo antes del commit de creación del repositorio.
...
- `Bases de conocimiento/4x4/EvaluacionMallas`$n$`.txt`: son generados mediante los scripts `funcionDeEvaluacion*.pl`.
  El archivo `EvaluacionMallas.txt` es simplemente la unión de estos.

- `Bases de conocimiento/4x4/LimitesPasos`$n$`.txt`: son generados mediante los scripts `funcionDeEvaluacion*.pl`.
  El archivo `LimitesPasos.txt` es simplemente la unión de estos.
...
Para verificar la generación correcta de los archivos, se puede consultar el archivo `.csv`, agregado después del commit de creación del repositorio, donde se indica el nombre y tamaño de todos los archivos del directorio de trabajo en su estado anterior al commit de creación del repositorio.
...
- [ ] Añadir el `.sh` que se usó para tomar las muestras de archivos en masa.
  Este archivo solo permanecerá en el índice hasta el siguiente commit, o bien, hasta el inicio de la segunda ronda de commits.
...
- [ ] En este punto solo deben quedar en el índice los elementos esenciales del proyecto, ya que el último commit de la segunda ronda será el que contenga el proyecto totalmente corregido y actualizado.
...
- [ ] Modificar rutas de scripts de acuerdo a nueva estructura de carpetas para que tomen sus entradas y generen sus salidas en las rutas correctas.
