...
void escribirMallas1(ofstream& archivo, size_t cont, int** listaVectores, int kk, int maxMuestras, double filtradasCaso, double fraccionMax){

	double fraccion = (1.0*(casosAux[kk][0] + casosAux[kk][1])/sumaN)/fraccionMax;
	bool tomarTodas = (fraccion*fraccionMax*maxMuestras >= filtradasCaso ? true : false);

	for(int i=0; i<=cont-1; i++){

		if(tomarTodas or ((double) 1.0*rand()/RAND_MAX)<=fraccion){
...
void escribirMallas2(ofstream& archivo, size_t cont, int** listaVectores, int kk, int i2, int maxMuestras, double filtradasCaso, double fraccionMax){
...
#pragma omp parallel for private(k,malla,cont1,cont,contAux,matriz22,matriz33,matriz44,matriz55,matriz66,matriz77,matriz88,archivosSalida,nombreArchivo,i2,t1,t2,t1_2) firstprivate(similarQ) schedule(dynamic)
for(k=0; k<=casosAux.size()-1; k++){
	...
	size_t perm = permutacionesPorCaso[{casosAux[k][0],casosAux[k][1]}];
	double fraccion = 1.0*max/sumaN;
	double fracc = ((double) 1.0*fraccion*maxMuestras/perm);
	int aceptar = 0;
	bool tomarTodas = ((double) fraccion*maxMuestras >= perm/8.0 ? true : false);
	bool aceptada = 0;
	size_t comp = (size_t) ceil(1.5*fraccion*maxMuestras);
	...
	printf("(Hilo %d) Caso %d,%d. Permutaciones = %u\n", omp_get_thread_num(), casosAux[k][0], casosAux[k][1], (size_t) perm);
	
	sort(malla, malla + espaciosMalla);
	do{
		aceptada = ((double) 1.0*rand()/RAND_MAX)<=fracc;
		
		if((tomarTodas or aceptada) or aceptar){
		...
		if(!similarQ){copy(malla, malla+espaciosMalla, listaVectores[cont]); cont++; if(!aceptada){aceptar--;}}
		else if(!(!aceptada and aceptar)){aceptar++;}
		//#pragma omp atomic
		//contUniv++;
	...
	escribirMallas1(archivosSalida, cont, listaVectores, k, maxMuestras, perm/8.0, fraccion);
	...
	if(cambiosPorCaso[k].size()>0){
		...
		for(i2=0; i2<=cambiosPorCaso[k].size()-1; i2++){
			...
			archivosSalida.open(nombreArchivo);
			escribirMallas2(archivosSalida, cont, listaVectores, k, i2, maxMuestras, perm/8.0, fraccion);
			archivosSalida.close();
		}
	}
