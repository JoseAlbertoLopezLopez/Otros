...
sumAux([], Sum, Sum).
sumAux([X|T], Acc, Sum) :- Acc1 is Acc+X, sumAux(T,Acc1,Sum).
...
pasos(MatrizIn, I/J, Pasos) :- 
	...
	caminoMasCorto(MatrizIn,I,J,Camino), length(Camino,Pasos)), !.

pasosElementos(MatrizIn, Celdas, SumaPasos) :-

	maplist(pasos(MatrizIn), Celdas, PasosPorElemento),
	sum(PasosPorElemento, SumaPasos).

evaluar(Celdas,N1/N2,N1/N2/Min) :- 

	% Método para leer las bases de casos separadas.
	string_concat(N1,",",S3),
	string_concat(S3,N2,S4),
	string_concat("Archivos casos/",S4,S5),
	[S5],
	
	string_concat("malla",N1,S1),
	string_concat("_",N2,S2),
	string_concat(S1,S2,S),
	term_string(Functor,S),
	findall(SumaPasos, evaluarAux(Celdas,Functor,SumaPasos), L), 
	unload_file(S5),
	delete(L, -1, L2),
	length(L2,N),
	(N==0,Min= -1,!;
	min_member(Min,L2)),
	write("Caso "), write(N1), write(","), write(N2), write("  "), (Min== -1, writeln("Fracaso"), ! ; writeln("Éxito")).
	
evaluarAux(Celdas,Functor,SumaPasos) :- 

	compound_name_arguments(C,Functor,[MatrizIn]),
	C, %malla(MatrizIn),
	(pasosElementos(MatrizIn, Celdas, SumaPasos) ; SumaPasos= -1).
%	escribirResultado(MatrizIn2, MatrizOut, SumaPasos, PromedioPasos, MinPasos, MaxPasos, Stream, N1, N2).

listaResultado(Lista) :-

	casos(L),
	dimensionesMallas(N,M),
	findall(I/J, (I in 1..N, J in 1..M, indomain(I), indomain(J)), Celdas),
	concurrent_maplist(evaluar(Celdas),L,Lista),
	escribirLimitesPasos(Lista).
	
escribirLimitesPasos(Lista) :- 

	open("LimitesPasos.txt", write, Stream), 
	escribirLinea(Lista,Stream),
	close(Stream).

%escribirLinea([N1/N2/Min],Stream) :- 
%	write(Stream,N1), write(Stream,','), write(Stream,N2), write(Stream," "), writeln(Stream,Min), !.
escribirLinea([N1/N2/Min|L],Stream) :- 

	write(Stream,N1), write(Stream,','), write(Stream,N2), write(Stream," "), writeln(Stream,Min),
	escribirLinea(L,Stream), !.
...
% [SumaPasos,N1,N2]	
resultadosVentana(ListaSumas) :- findall([SumaPasos,N1,N2], (mallaV(MatrizIn), contarClases(MatrizIn, N1, N2), funcionDeEvaluacion(MatrizIn, SumaPasos)), ListaSumas).
resultadosRefuerzo(ListaSumas) :- findall([SumaPasos,N1,N2], (mallaR(MatrizIn), contarClases(MatrizIn, N1, N2), funcionDeEvaluacion(MatrizIn, SumaPasos)), ListaSumas).
