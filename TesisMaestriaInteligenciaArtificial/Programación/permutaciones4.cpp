// Declarando el vector de filtradas dentro del for.
...
void filtradasPorCaso(){
	...
	for(int i=n*m; i>=limDownNs; i--){
		for(int j=(i>=limUpNs ? i-limUpNs : 0); j<=(i>=limUpNs ? limUpNs : i); j++){
			...
			filtradasCaso[{i-j, j}] = casosAux/6;
...
void escribirMallas1(ofstream& archivo, unsigned long cont, int listaVectores[][espaciosMalla], int n, int m, int kk){
...
void escribirMallas2(ofstream& archivo, unsigned long cont, int listaVectores[][espaciosMalla], vector<vector<int>> &nombresCasos, int n, int m, vector<int> cambio, int i2){
	...
	for(int i=0; i<=cont-1; i++){
		...
		for(int j=0; j<=n-1; j++){
			...
			for(int k=0; k<=m-1; k++){
				...
				archivo << (listaVectores[i][j*n+k]==1 ? cambio[0] : (listaVectores[i][j*n+k]==2 ? cambio[1] : cambio[2])) << (k==m-1 ? ",0" : ",");
			}
			archivo << (j==n-1 ? "]" : "],");
		}   
		archivo << ",[" << repeat("0,",m+1) << "0]])." << endl;   
...
//Cont_Univ = 44705
#pragma omp parallel for private(k,malla,cont,contAux,matriz22,matriz33,matriz44,matriz55,matriz66,matriz77,matriz88,archivosSalida,nombreArchivo,i2,t1,t2,t1_2) firstprivate(similarQ) // nowait
for(k=0; k<=casosAux.size()-1; k++){

	int listaVectores[filtradasCaso[{casosAux[k][0],casosAux[k][1]}]][espaciosMalla];
	...
	printf("(Hilo %d) Filtradas (%d, %d) x %d = %u\n", omp_get_thread_num(), casosAux[k][0], casosAux[k][1], (int) (cambios.size()+1), (size_t) cont);
