...
double puntuacionFrontera(int r, int c, array<array<int, 2>, 2> &ventana){
	...
	if(!elementosLiberados[{r-1,c-1}]){
		if(malla[r-2][c-1]>=0) puntuacion += puntuacionInteraccion(ventana[0][0], malla[r-2][c-1]);
		if(malla[r-1][c-2]>=0) puntuacion += puntuacionInteraccion(ventana[0][0], malla[r-1][c-2]);
	}
	
	if(!elementosLiberados[{r-1,c}]){
		if(malla[r-2][c]>=0) puntuacion += puntuacionInteraccion(ventana[0][1], malla[r-2][c]);
		if(malla[r-1][c+1]>=0) puntuacion += puntuacionInteraccion(ventana[0][1], malla[r-1][c+1]);
	}
	
	if(!elementosLiberados[{r,c-1}]){
		if(malla[r][c-2]>=0) puntuacion += puntuacionInteraccion(ventana[1][0], malla[r][c-2]);
		if(malla[r+1][c-1]>=0) puntuacion += puntuacionInteraccion(ventana[1][0], malla[r+1][c-1]);
	}
	
	if(!elementosLiberados[{r,c}]){
		if(malla[r+1][c]>=0) puntuacion += puntuacionInteraccion(ventana[1][1], malla[r+1][c]);
		if(malla[r][c+1]>=0) puntuacion += puntuacionInteraccion(ventana[1][1], malla[r][c+1]);
	}
...
double puntuacionPotencial_1(int r, int c, array<array<int, 2>, 2> &ventana){
	...
	if((c<m and r==2) and !elementosLiberadosAux[{r-1,c}]){
	cout << "Hola1 = " << r << "," << c << endl;
		elem = ventana[0][1];
		for(int i=0; i<=numClases; i++){
	...
	if(r<n and !elementosLiberadosAux[{r,c-1}]){
	cout << "Hola2 = " << r << "," << c << endl;
		elem = ventana[1][0];
		for(int i=0; i<=numClases; i++){
	...
	if(elementosLiberadosAux[{r,c}]) frontera=0;
...
double premiarLibertad(int r, int c, array<array<int, 2>, 2> &ventana){
	...
	double puntuacion = 0.0;
	double puntuacionAux1;
	double puntuacionAux2;

	if(!elementosLiberadosAux[{r-1,c-1}]){
	
		if((ventana[0][0] > malla[r-2][c-1] and ventana[0][0] > ventana[1][0]) or (ventana[0][0] > malla[r-1][c-2] and ventana[0][0] > ventana[0][1])){

			elementosLiberadosAux[{r-1,c-1}] = true;

			puntuacion += 5.0;
			puntuacionAux1 = puntuacionInteraccion(ventana[0][0], malla[r-2][c-1]) + puntuacionInteraccion(ventana[0][0], ventana[1][0]);
			puntuacionAux2 = puntuacionInteraccion(ventana[0][0], malla[r-1][c-2]) + puntuacionInteraccion(ventana[0][0], ventana[0][1]);
		
			puntuacion += (puntuacionAux1 > puntuacionAux2 ? puntuacionAux1 : puntuacionAux2);
		}
		else{
			...
			puntuacion += puntuacionAux1 + puntuacionAux2;
		}
	}
	
	if(!elementosLiberadosAux[{r-1,c}]){
	
		if((ventana[0][1] > malla[r-2][c] and ventana[0][1] > ventana[1][1]) or ((ventana[0][1] > malla[r-1][c+1] and ventana[0][1] > ventana[0][0]) and (malla[r-1][c+1]>=0))){
	
			elementosLiberadosAux[{r-1,c}] = true;
	
			puntuacion += 5.0;
			puntuacionAux1 = puntuacionInteraccion(ventana[0][1], malla[r-2][c]) + puntuacionInteraccion(ventana[0][1], ventana[1][1]);
			if(malla[r-1][c+1]>=0) puntuacionAux2 = puntuacionInteraccion(ventana[0][1], malla[r-1][c+1]) + puntuacionInteraccion(ventana[0][1], ventana[0][0]);
			else puntuacionAux2 = 0;
			...
			puntuacionAux1 = puntuacionInteraccion(ventana[0][1], malla[r-2][c]) + puntuacionInteraccion(ventana[0][1], ventana[1][1]);
			if(malla[r-1][c+1]>=0) puntuacionAux2 = puntuacionInteraccion(ventana[0][1], malla[r-1][c+1]) + puntuacionInteraccion(ventana[0][1], ventana[0][0]);
			else puntuacionAux2 = puntuacionInteraccion(ventana[0][1], ventana[0][0]);
			
			puntuacion += puntuacionAux1 + puntuacionAux2;
		}
	}
	
	if(!elementosLiberadosAux[{r,c-1}]){
		
		if(((ventana[1][0] > malla[r+1][c-1] and ventana[1][0] > ventana[0][0]) and (malla[r+1][c-1]>=0)) or (ventana[1][0] > malla[r][c-2] and ventana[1][0] > ventana[1][1])){

			elementosLiberadosAux[{r,c-1}] = true;

			puntuacion += 5.0;
			puntuacionAux1 =  puntuacionInteraccion(ventana[1][0], malla[r][c-2]) + puntuacionInteraccion(ventana[1][0], ventana[1][1]);
			if(malla[r+1][c-1]>=0) puntuacionAux2 = puntuacionInteraccion(ventana[1][0], malla[r+1][c-1]) + puntuacionInteraccion(ventana[1][0], ventana[0][0]);
			else puntuacionAux2 = 0;
			...
			puntuacionAux1 = puntuacionInteraccion(ventana[1][0], malla[r][c-2]) + puntuacionInteraccion(ventana[1][0], ventana[1][1]);
			if(malla[r+1][c-1]>=0) puntuacionAux2 = puntuacionInteraccion(ventana[1][0], malla[r+1][c-1]) + puntuacionInteraccion(ventana[1][0], ventana[0][0]);
			else puntuacionAux2 = puntuacionInteraccion(ventana[1][0], ventana[0][0]);
			
			puntuacion += puntuacionAux1 + puntuacionAux2;
		}
	}
	
	if(((ventana[1][1] > malla[r+1][c] and ventana[1][1] > ventana[0][1]) and malla[r+1][c]>=0) or ((ventana[1][1] > malla[r][c+1] and ventana[1][1] > ventana[1][0]) and malla[r][c+1]>=0)){

		elementosLiberadosAux[{r,c}] = true;

		puntuacion += 5.0;
		if(malla[r+1][c]>=0) puntuacionAux1 = puntuacionInteraccion(ventana[1][1], malla[r+1][c]) + puntuacionInteraccion(ventana[1][1], ventana[0][1]);
		else puntuacionAux1 = 0;
		if(malla[r][c+1]>=0) puntuacionAux2 = puntuacionInteraccion(ventana[1][1], malla[r][c+1]) + puntuacionInteraccion(ventana[1][1], ventana[1][0]);
		else puntuacionAux2 = 0;
		...
		if(malla[r+1][c]>=0) puntuacionAux1 = puntuacionInteraccion(ventana[1][1], malla[r+1][c]) + puntuacionInteraccion(ventana[1][1], ventana[0][1]);
		else puntuacionAux1 = puntuacionInteraccion(ventana[1][1], ventana[0][1]);
		if(malla[r][c+1]>=0) puntuacionAux2 = puntuacionInteraccion(ventana[1][1], malla[r][c+1]) + puntuacionInteraccion(ventana[1][1], ventana[1][0]);
		else puntuacionAux2 = puntuacionInteraccion(ventana[1][1], ventana[1][0]);
		...
		for(int j=0; j<=dicMallasBaseEsquinas[formasPosibles[i]].size()-1; j++){
			...
			if(minoria >= minoriaNecesaria()){
				...
				if(encajan(r, c, ventana)){
					elementosLiberadosAux = elementosLiberados;
					puntuacion = premiarLibertad(r, c, ventana) + puntuacionPotencial_1(r, c, ventana);
					anadirForma(ventana, puntuacion);
					if(formasPosiblesPuntuacion.size() > 0) for(auto &elem:formasPosiblesPuntuacion[formasPosiblesPuntuacion.size()-1]){cout << elem << " ";}; cout << endl;
				}
			}
...
void verificarLiberados(int r, int c){
	...
	elementosLiberados[{r-1,c-1}] = true;
	
	if((malla[r-1][c] > malla[r-2][c] and malla[r-1][c] > malla[r][c]) or ((malla[r-1][c] > malla[r-1][c-1] and malla[r-1][c] > malla[r-1][c+1]) and malla[r-1][c+1]>=0))
	elementosLiberados[{r-1,c}] = true;
	
	if((malla[r][c-1] > malla[r][c-2] and malla[r][c-1] > malla[r][c]) or ((malla[r][c-1] > malla[r-1][c-1] and malla[r][c-1] > malla[r+1][c-1]) and malla[r+1][c-1]>=0))
	elementosLiberados[{r,c-1}] = true;
	
	if(((malla[r][c] > malla[r][c-1] and malla[r][c] > malla[r][c+1]) and malla[r][c+1]>=0) or ((malla[r][c] > malla[r-1][c] and malla[r][c] > malla[r+1][c]) and malla[r+1][c]>=0)) 
	elementosLiberados[{r,c}] = true;
}
