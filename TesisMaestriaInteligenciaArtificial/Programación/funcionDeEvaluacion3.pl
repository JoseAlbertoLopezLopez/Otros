...
funcionDeEvaluacion(MatrizIn, SumaPasos) :-

	findall(Pasos, pasosElementos(MatrizIn,Pasos), MatrizOut), 
	(\+ member(-1, MatrizOut),
	%delete(MatrizOut, 0, MatrizOutAux),
	sum(MatrizOut, SumaPasos) ; 
	SumaPasos = -1), !.

funcionDeEvaluacionOld(MatrizIn, SumaPasos, PromedioPasos, MinPasos, MaxPasos, MatrizOut) :-
...
evaluar(N1/N2,N1/N2/Min) :- 
	...
	findall(SumaPasos, evaluarAux(Functor,SumaPasos), L), 
	unload_file(S5),
	delete(L, -1, L2),
	length(L2,N),
	(N==0,Min= -1,!;
	min_member(Min,L2)),
	write("Caso "), write(N1), write(","), writeln(N2).
	
evaluarAux(Functor,SumaPasos) :- 
...
listaResultado(Lista) :-
	...
	casos(L),
	concurrent_maplist(evaluar,L,Lista),
