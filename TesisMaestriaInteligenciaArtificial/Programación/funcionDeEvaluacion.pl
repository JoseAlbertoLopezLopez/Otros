...
sujetable(MatrizIn, I, J, Formas) :- findall(X, sujetableAux(MatrizIn,I,J,X), Formas), length(Formas,N), N>0.

sujetableAux(MatrizIn, I, J, Forma) :- eij(MatrizIn,I,J,Elem), J2 is J-1, J3 is J+1, eij(MatrizIn,I,J2,L), eij(MatrizIn,I,J3,R), R<Elem, L<Elem, Forma=h.
sujetableAux(MatrizIn, I, J, Forma) :- eij(MatrizIn,I,J,Elem), I2 is I-1, I3 is I+1, eij(MatrizIn,I2,J,T), eij(MatrizIn,I3,J,B), B<Elem, T<Elem, Forma=v.
...
caminoPosible(MatrizIn, I, J, CaminoAux, Camino) :- sujetable(MatrizIn,I,J,_), append(CaminoAux, [I/J], CaminoInv), reverse(CaminoInv, Camino).

caminoPosible(MatrizIn, I, J, CaminoAux, Camino) :- 

	eij(MatrizIn,I,J,Elem), Elem \= 0,
	\+ sujetable(MatrizIn,I,J,_), 
	\+ member(I/J,CaminoAux), 
	append(CaminoAux,[I/J],CaminoAux2), 
	I2 is I-1, I3 is I+1, 
	eij(MatrizIn,I2,J,O), O<Elem, 
...
caminoPosible(MatrizIn, I, J, CaminoAux, Camino) :- 
	...
	append(CaminoAux, [I/J], CaminoAux2), 
	I2 is I-1, I3 is I+1,
	eij(MatrizIn,I2,J,O1), O1>=Elem, 
	eij(MatrizIn,I3,J,O2), O2>=Elem, 
...
caminoPosible(MatrizIn, I, J, CaminoAux, Camino) :- 
	...
	append(CaminoAux, [I/J], CaminoAux2), 
	J2 is J-1, J3 is J+1,
	eij(MatrizIn,I,J2,O1), O1>=Elem, 
	eij(MatrizIn,I,J3,O2), O2>=Elem, 
...
evaluar :- 
	...
	findall(N1N2SumaPasos, evaluarAux(N1N2SumaPasos,Stream), L), 
	close(Stream),
...
evaluarAux(N1/N2/SumaPasos, Stream) :- 
...
escribirResultado(MatrizIn, MatrizOut, SumaPasos, PromedioPasos, MinPasos, MaxPasos, Stream, OcurrenciasClase1, OcurrenciasClase2) :-

	length(MatrizIn,R1), R is R1-2,
	nth0(0,MatrizIn,Row),
	length(Row,C1), C is C1-2,
	flatten(MatrizIn,MatrizIn2),
	count(1, MatrizIn2, OcurrenciasClase1), count(2, MatrizIn2, OcurrenciasClase2),
	write(Stream, R), write(Stream, "x"), write(Stream, C), write(Stream, ","), write(Stream, OcurrenciasClase1), write(Stream, ","), write(Stream, OcurrenciasClase2), nl(Stream),
	escribirLista(Stream, MatrizIn2),
...
% [SumaPasos,N1,N2]	
resultadosVentana(ListaSumas) :- findall([SumaPasos,N1,N2], (mallaV(MatrizIn), contarClases(MatrizIn, N1, N2), funcionDeEvaluacion(MatrizIn, SumaPasos, PromedioPasos, MinPasos, MaxPasos, MatrizOut)), ListaSumas).
resultadosRefuerzo(ListaSumas) :- findall([SumaPasos,N1,N2], (mallaR(MatrizIn), contarClases(MatrizIn, N1, N2), funcionDeEvaluacion(MatrizIn, SumaPasos, PromedioPasos, MinPasos, MaxPasos, MatrizOut)), ListaSumas).
