...
const array<int,numClases> Ns {{48,16}};
...
array<array<int, 3>, (n-2)*(m-2)> listaPotencialesImposibles; 
...
vector<array<int, 5>> formasPosiblesPuntuacion;		// Formas posibles de llenar la ventana junto con su puntuación.
...
array<array<int,2>,2>& llenarVentana(int r, int c){
	...
	eliminarImposiblesPotenciales2(r, c, potencialesCuadrosImposibles, ventanaAux);
	
	//imprimirMatriz(ventana);
